...
print("About to load")
loader = wf.Loader
# TODO: need to change file name and dir to your local path
...
ind,dataset = splitter.k_fold(data, n_splits = n)

indices = list()
for train,test in ind:
    indices.append((train,test))

# conduct multiple cross validation and plots for every cv
for r in range(repetition):
    ...
    scores,predictions,test_datasets = wf.Run.cv(dataset,indices, model = 'GC',model_args = args,n_splits = n, metrics = ['AAD', 'RMSE', 'MAE', 'R2'])
